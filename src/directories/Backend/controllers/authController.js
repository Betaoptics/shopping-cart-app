...
const refresh = (req, res) => {
    ...
    if (!cookies?.jwt) return res.status(401).json({ message: 'Unauthorized authController' })
    ...
    jwt.verify(
        ...
        async (err, decoded) => {
            ...
            if (!foundUser) return res.status(401).json({ message: 'Unauthorized authController' })
